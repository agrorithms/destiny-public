 ...
 *
 * **Restart versus collapse is a count, not an adjective.** The 7:47 mean #81 quotes is
 * pulled up by a tail of hour-long Runs (the median is about three minutes), so the panel
    ...
    return (
        <section aria-labelledby="archive-resets-heading" className="space-y-3">
            ...
            {nonClears === 0 ? (
                ...
                // Run and nothing else. Four "none" sentences would read as a broken panel.
                <p className="ui-text-secondary text-sm leading-6">
                ...
                <div data-testid="archive-resets" className="ui-card space-y-3 rounded-md border px-4 py-3">
                    ...
                    <p className="ui-text-secondary text-sm leading-6">
                        <UnpinnedClearSentence outcomes={outcomes} />
                    </p>
                    <p className="ui-text-secondary text-sm leading-6">
                    ...
                    <p className="ui-text-secondary text-xs leading-5 tabular-nums">
                        ...
                        {outcomes.unpinnedClears.toLocaleString()} +{' '}
...
/** #81's "pre-pin clears" — which are in fact all after the pin; see getNonClearRuns. */
function UnpinnedClearSentence({ outcomes }: { outcomes: ArchiveNonClearRuns }) {
    const n = outcomes.unpinnedClears;

    if (n === 0) {
        return <>No finished run in this range was left out by how the 10,000 is counted.</>;
    }

    return (
        <>
            <Figure>{n.toLocaleString()}</Figure> {plural(n, 'run', 'runs')} he finished that began
            at the first encounter but that Bungie&apos;s own report did not mark as started from
            the beginning. The stricter count behind the 10,000 leaves{' '}
            {n === 1 ? 'it' : 'them'} out; the looser one includes {n === 1 ? 'it' : 'them'}.
        </>
    );
}
    ...
    return (
        <>
            ...
            {/* Only where it is true: October 2020 is 5 Checkpoint Runs of 9, and saying
                ...
                figure. Unfiltered it is 8 of 13,420. */}
